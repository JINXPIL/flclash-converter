...
В этой папке находится готовый профиль маршрутизации для клиентов на базе ядра Sing-box / Xray (NekoRay, NekoBox). 
Он настроен для комфортного обхода блокировок: российские сервисы работают напрямую, заблокированные — через прокси, а мусорный трафик блокируется.

## 📥 Как установить

1. Откройте файл `Throne_INSTALL.DEEPLINK` в этой папке.
2. Скопируйте ссылку, которая начинается с `throne://route/...`
3. В NekoRay перейдите в **Настройки -> Маршруты**, нажмите **Импорт** и вставьте ссылку.

---

## 📜 Что внутри? (Список правил)

Здесь представлен список доменов и IP-адресов, которые вшиты в профиль.

### 🟢 Напрямую (Direct / Bypass)
*Эти сайты и IP-адреса идут в обход VPN (через вашего провайдера). Здесь находятся Госуслуги, российские банки, маркетплейсы, сервисы Яндекса и ВК.*

<details>
<summary>Нажмите, чтобы развернуть список Direct</summary>
...
Сайты, которые принудительно открываются через VPN.
...
Эти сайты блокируются на уровне программы. Сюда включены известные трекеры
слежки, телеметрия ОС и смартфонов, а также рекламные сети.
...
Примечание: Если сайта нет ни в одном из списков (неизвестный зарубежный
ресурс), по умолчанию он отправляется через прокси.


***

**Что делают теги `<details>` и `<summary>`?**
Они создают аккуратные "спойлеры". Списки сайтов очень длинные, и если вывалить их просто так, страница на GitHub будет бесконечной. А благодаря этим тегам списки будут скрыты, и пользователь сможет кликнуть на "Нажмите, чтобы развернуть...", чтобы посмотреть их содержимое.
