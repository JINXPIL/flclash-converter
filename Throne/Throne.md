...
Он настроен для комфортного и безопасного обхода блокировок:
* 🟢 **Российские сервисы** работают напрямую (без VPN).
* 🔵 **Заблокированные сайты** открываются через прокси.
* 🔴 **Мусорный трафик** (реклама, слежка) блокируется на уровне программы.

---

## 📥 Как установить профиль

1. Откройте файл `Throne_INSTALL.DEEPLINK` в этой папке.
2. Скопируйте всю длинную строку, которая начинается с `throne://route/...` *(выделите её и нажмите Ctrl+C)*.
3. Откройте программу **Throne** (или NekoRay) на вашем ПК.
4. Перейдите в верхнем меню: **Настройки ➔ Маршруты**.
5. Нажмите кнопку **Импорт** (или *Импорт из буфера обмена*) и вставьте скопированную ссылку.
6. Нажмите **ОК**. Готово! 🎉

---

## 📜 Что внутри? (Списки правил)

Ниже представлен полный список доменов и IP-адресов, которые вшиты в этот профиль. *Списки скрыты под спойлерами для удобства чтения.*

### 🟢 Напрямую (Direct / Bypass)
> Эти сайты и IP-адреса идут в обход VPN (через вашего обычного провайдера). Здесь находятся Госуслуги, российские банки, маркетплейсы, сервисы Яндекса и ВК. Это обеспечивает максимальную скорость загрузки и защищает от блокировок банковских аккаунтов из-за подозрительных IP-адресов.

<details>
<summary><b>👀 Нажмите, чтобы развернуть список (Direct)</b></summary>
...
Заблокированные и зарубежные ресурсы (нейросети, соцсети, видеохостинги),
которые принудительно открываются через ваш VPN-туннель.
...
Эти сайты жестко блокируются на уровне самой программы. Сюда включены известные
трекеры слежки, метрики, телеметрия ОС и смартфонов, а также рекламные сети. Это
экономит ваш трафик и ускоряет загрузку страниц.
...
[!NOTE] Что будет с остальными сайтами? Если вы откроете сайт, которого нет ни в
одном из списков выше (например, любой неизвестный или новый зарубежный ресурс),
сработает встроенное правило «Outbound по умолчанию: proxy», и этот сайт
безопасно откроется через ваш VPN.
