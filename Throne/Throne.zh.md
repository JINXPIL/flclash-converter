<p align="center">
  ...
  <a href="Throne.zh.md"><b>🇨🇳 简体中文</b></a> •
  <a href="Throne.zh-TW.md">🇹🇼 繁體中文</a> •
  <a href="Throne.fa.md">🇮🇷 فارسی</a> •
  <a href="Throne.es.md">🇪🇸 Español</a>
</p>

# 👑 Throne 路由配置文件 (NekoRay / NekoBox)

此文件夹包含适用于基于 Sing-box / Xray 内核客户端（**Throne, NekoRay, NekoBox**）的现成路由配置文件。
该配置旨在提供舒适且安全的绕过审查体验：
* 🟢 **本地/俄罗斯服务** 直接连接（不通过 VPN）。
* 🔵 **被屏蔽的网站** 通过代理打开。
* 🔴 **垃圾流量**（广告、跟踪器）在程序层面被拦截。

---

## 📥 如何安装配置文件

1. 打开此文件夹中的 `Throne_INSTALL.DEEPLINK` 文件。
2. 复制以 `throne://route/...` 开头的整段长字符串 *（选中它并按 Ctrl+C）*。
3. 在您的电脑上打开 **Throne**（或 NekoRay）程序。
4. 转到顶部菜单：**设置 ➔ 路由** (Preferences ➔ Routing)。
5. 点击 **导入**（或 *从剪贴板导入*）按钮，并粘贴复制的链接。
6. 点击 **确定**。完成！🎉
...
以下是此配置文件中嵌入的完整域名和 IP 地址列表。*为方便阅读，列表已隐藏在折叠面板下。*

### 🟢 直连 (Direct / Bypass)
> 这些网站和 IP 地址绕过 VPN（通过您的常规网络提供商）。这包括政府服务、本地银行、市场、Yandex 和 VK 服务等基础服务。这可确保最快的加载速度，并防止由于可疑的 IP 地址而导致银行账户被封锁。

<details>
<summary><b>👀 点击展开列表 (直连)</b></summary>
...
### 🔵 代理 (Proxy)
> 被屏蔽的和国外的资源（AI、社交网络、视频托管），被强制通过您的 VPN 隧道打开。

<details>
<summary><b>👀 点击展开列表 (代理)</b></summary>
...
### 🔴 拦截 (Block / Reject)
> 这些网站在程序级别被严格屏蔽。这包括已知的追踪器、统计数据、操作系统和智能手机遥测，以及广告网络。这可以节省您的流量并加快页面加载速度。

<details>
<summary><b>👀 点击展开列表 (拦截)</b></summary>
...
> [!NOTE]
> **其他网站会怎样？**
> 如果您打开一个不在上述任何列表中的网站（例如，任何未知或新的国外资源），将触发内置的规则 **“默认出站：代理”** (Default outbound: proxy)，并且此网站将安全地通过您的 VPN 打开。
```
