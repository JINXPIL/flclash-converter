<p align="center">
  <a href="/README.en.md">🇺🇸 English</a> •
  <a href="/README.md"><b>🇷🇺 Русский</b></a> •
  <a href="/README.zh.md">🇨🇳 简体中文</a> •
  <a href="/README.zh-TW.md">🇹🇼 繁體中文</a> •
  <a href="/README.fa.md">🇮🇷 فارسی</a> •
  <a href="/README.es.md">🇪🇸 Español</a>
</p>

<p align="center">
  <a href="https://github.com/jinxpil/flclash-converter/releases">
    <img src="https://img.shields.io/github/v/release/jinxpil/flclash-converter.svg?style=for-the-badge&color=blue" alt="Release">
  </a>
  <a href="https://github.com/jinxpil/flclash-converter/releases/latest">
    <img src="https://img.shields.io/github/downloads/jinxpil/flclash-converter/total.svg?style=for-the-badge&color=brightgreen" alt="Downloads">
  </a>
  <a href="https://opensource.org/licenses/MIT">
    <img src="https://img.shields.io/badge/license-MIT-blue.svg?style=for-the-badge" alt="License">
  </a>
  <a href="https://github.com/jinxpil/flclash-converter/stargazers">
    <img src="https://img.shields.io/github/stars/jinxpil/flclash-converter.svg?style=for-the-badge&color=yellow" alt="Stars">
  </a>
</p>

<h1 align="center">⚡ Network Builder v17.02 ULTRA</h1>

<p align="center">
  <b>Экстремально мощный и 100% локальный веб-инструмент для управления гигантскими базами прокси-серверов.</b><br>
  <i>Парсинг, конвертация и глубокая оптимизация конфигураций для FlClash, Exclave, Sing-box и Clash Meta.</i>
</p>

<p align="center">
  <a href="https://jinxpil.github.io/flclash-converter/">
    <img src="https://img.shields.io/badge/🚀_ОТКРЫТЬ_ULTRA--ВЕРСИЮ-0052FF?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Open Web Version">
  </a>
</p>

> [!IMPORTANT]
> **Zero-Trust & Privacy First.** Все вычисления производятся строго локально в вашем браузере. Никакие данные не покидают ваше устройство. Инструмент создан исключительно в образовательных целях для глубокого изучения сетевых протоколов.

---

### 🔥 Что нового в версии 17.02 ULTRA?

- 🚀 **Extreme Parser Engine:** Полная оптимизация кода для работы с **10,000+ узлами** одновременно. Плавный интерфейс даже при гигантских объемах данных.
- 🌍 **ULTRA Localization:** Глубокая поддержка 6 языков (RU, EN, ZH, FA, ES).
- 🛡️ **Система QuotaGuard:** Интеллектуальная защита от переполнения памяти браузера.
- 🧠 **Smart Duplicate Filter:** Автоматическое удаление дубликатов серверов по паре IP:Port.
- 🛠️ **Refined Diagnostics:** Обновленный логгер событий и исправление ошибок синтаксиса.

---

### 🔌 Поддерживаемые протоколы

<table align="center" width="100%">
  <tr>
    <td align="center">🛡️ <b>VLESS / VMess</b><br>Reality & Vision</td>
    <td align="center">⚡ <b>Hysteria 2</b><br>Brutal Speed</td>
    <td align="center">🐎 <b>Trojan</b><br>Stealth Protocol</td>
  </tr>
  <tr>
    <td align="center">🔒 <b>WireGuard</b><br>Native VPN</td>
    <td align="center">🌐 <b>SS / SSR</b><br>Shadowsocks</td>
    <td align="center">🔌 <b>TUIC / Socks5 / HTTP</b><br>Full Support</td>
  </tr>
</table>

---

### 🎯 Конфигурации и маршрутизация (Пресеты)

Чтобы не засорять страницу портянками кода, все готовые файлы настроек вынесены в отдельные файлы репозитория.

* 🌐 **Раздельное туннелирование для Happ** — Оптимизированный пресет правил (Крым, СевГУ, обход ограничений, AdBlock), собранный через конструктор `routing.happ.su`. Предназначен **строго** для клиента Happ.
  * 📄 Ссылка на файл конфигурации: [`./rus_vp_happ.json`](./rus_vp_happ.json)
* 🛠️ **Оптимизированный YAML-конфиг (Mihomo/Clash)** — v79.0 Ultimate со строгими TLS-отпечатками и Anti-DPI.
  * 📄 Ссылка на файл конфигурации: [`./GL_Crimea_ipv6_yan(9.35).yml`](./GL_Crimea_ipv6_yan(9.35).yml)

---

### 📥 Рекомендуемые клиенты (ULTRA Compatible)

<details>
<summary><b>🤖 Android</b></summary>

- [v2RayTun](https://v2raytun.com/) — На базе ядра Xray Core.
- [FlClash](https://github.com/chen08209/FlClash) — Кроссплатформенный клиент.
- [Incy](https://incy.cc/) — Быстрый импорт в одно касание.
- [Happ](https://www.happ.su/main) — Прокси-утилита под готовые JSON-маршруты.
- [Exclave](https://github.com/ExclaveNetwork/Exclave) — Актуальный аналог NekoBox.
- [Sing-box](https://github.com/SagerNet/sing-box) — Официальное чистое ядро.
- [Hiddify App](https://github.com/hiddify/hiddify-app) — Универсальный клиент.
</details>

<details>
<summary><b>🍏 iOS</b></summary>

- [v2RayTun](https://v2raytun.com/) — На базе ядра Xray Core.
- [Incy](https://incy.cc/) — Клиент в App Store.
- [Happ](https://www.happ.su/main) — Удобная прокси-утилита.
- [Karing](https://apps.apple.com/us/app/karing/id6472431552)
- [Stash](https://apps.apple.com/us/app/stash-rule-based-proxy/id1596063349)
</details>

<details>
<summary><b>💻 Desktop (Win/Mac/Linux)</b></summary>

- [v2RayTun](https://v2raytun.com/) — На базе ядра Xray Core.
- [Happ](https://www.happ.su/main) — Десктопная версия утилиты.
- [v2rayN](https://github.com/2dust/v2rayN) — Классика для Windows.
- [FlClash](https://github.com/chen08209/FlClash) — Быстрый GUI.
</details>

---

### 🛡️ Альтернативные инструменты (DPI Bypass)

* [Zapret](https://github.com/bol-van/zapret) — Глубокий обход DPI на уровне системы.
* [Zapret-discord-youtube](https://github.com/Flowseal/zapret-discord-youtube) — Скрипты под конкретные сервисы.
* [ByeByeDPI](https://github.com/romanvht/ByeByeDPI) — Локальный запуск на Windows.

---

### ⭐ Поддержка проекта

[![Stargazers over time](https://starchart.cc/jinxpil/flclash-converter.svg?variant=adaptive)](https://starchart.cc/jinxpil/flclash-converter)
