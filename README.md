...
Чтобы не засорять страницу портянками кода, все готовые пресеты вынесены в отдельные файлы репозитория. В них вшита продвинутая база GeoSite/GeoIP тегов для обхода ТСПУ/DPI, работы локальных сервисов (банки, СевГУ, госуслуги) без задержек и встроенный блокировщик рекламы.

#### 1. 🌐 Раздельное туннелирование для Happ (JSON)
Оптимизированный пресет правил (Крым, локальные ресурсы, обход ограничений, AdBlock), собранный для стабильной работы без обрывов сессий. Предназначен **строго** для клиента Happ.

* 👁️ **Посмотреть код:** [`rus_vp_happ.json`](./rus_vp_happ.json)
* 🔗 **Ссылка для импорта в клиент (скопируйте URL ниже):**
  ```text
  [https://raw.githubusercontent.com/JINXPIL/flclash-converter/refs/heads/main/rus_vp_happ.json](https://raw.githubusercontent.com/JINXPIL/flclash-converter/refs/heads/main/rus_vp_happ.json)
