...
[🇺🇸 Read in English](https://www.google.com/search?q=%23english-version) | [🇷🇺 Читать на Русском](https://www.google.com/search?q=%23russian-version) | [🇨🇳 简体中文](https://www.google.com/search?q=%23chinese-version)
...
* 📦 **Omni-Парсер:** Декодирование Base64-подписок и умный реверс-инжиниринг JSON-конфигов (вытягивает серверы, ключи Reality и SNI прямо из `outbounds`).
...
* 📱 **Fluid UI & i18n:** Mobile-first adaptive UI with auto-resizing text areas (expands to fit content) and built-in on-the-fly localization (RU, EN, CN) via `data-attributes`.
* 📦 **Omni-Parser:** Base64 subscription decoding and JSON config reverse-engineering (extracts nodes, Reality keys, and SNI directly from `outbounds`).
...
* 📱 **流畅的 UI & 多语言 (i18n):** 移动端优先的自适应界面，文本框智能自动调整大小（随内容撑开），内置基于数据属性的实时本地化切换（中/英/俄）。
