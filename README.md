...
> [!IMPORTANT]
> **Zero-Trust & Privacy First.** Все вычисления производятся строго локально в вашем браузере. Никакие данные не покидают ваше устройство. Инструмент создан исключительно в образовательных целях для глубокого изучения сетевых протоколов. Ограничения провайдеров, приводящие к Timeout при обновлении подписок, легко обходятся локальным копированием профилей.

---

### 🔮 Новинка: INCY Config Tool (Ultimate PWA)

Помимо глобального конвертера, в проекте доступна **специальная автономная среда разработки (IDE) для мобильных устройств**, предназначенная для хирургической правки профилей маршрутизации `incy://` и `happ://` прямо "на ходу".

* 📱 **Zero-Dependency & PWA:** Состоит из 1 файла. Не использует внешних библиотек. Работает 100% локально даже без интернета (можно вынести на рабочий стол телефона как нативное приложение).
* 🪄 **Обертка INCY-MINI:** Уникальный системный алгоритм сжатия ссылок (в 5+ раз) для обхода лимитов Telegram/WhatsApp на длину сообщений (с последующей авто-распаковкой).
* 🚀 **Direct Deep-Link:** Возможность запустить измененный профиль прямо из браузера в клиент одним тапом.
* 🧠 **Mobile Smart Editor:** Автоотступы (как в VS Code), локатор синтаксических ошибок JSON с автопрокруткой, Undo-перехватчик, менеджер форматов и полная история изменений профилей.

👉 **[Запустить INCY Config Tool](https://jinxpil.github.io/flclash-converter/INCY_Tool.html)**
...
### 🤝 Альтернативные базы маршрутизации

Если вы ищете другие варианты баз для сплит-туннелирования от комьюнити:
* [Roscomvpn-routing](https://github.com/hydraponique/roscomvpn-routing) — Отличный проект от *hydraponique* с регулярными обновлениями готовых конфигураций и правил маршрутизации под **INCY, HAPP** и **Clash Meta**.

---

### 🚦 Что внутри маршрутизации (Правила)

Надежный "хирургический" сплит-туннелинг, чтобы нужные ресурсы летали без задержек, а заблокированные — открывались через прокси.
...
*Если стандартные VPN-протоколы полностью блокируются оператором или ТСПУ, используйте локальный обход глубокого анализа пакетов или специализированные прокси-туннели:*

* [Zapret](https://github.com/bol-van/zapret) — Мощнейший и самый гибкий инструмент для глубокого обхода DPI на уровне системы.
* [Zapret2](https://github.com/bol-van/zapret2) — Обновленная и экспериментальная ветка скриптов мощнейшего обхода от `bol-van`.
* [Zapret-discord-youtube](https://github.com/Flowseal/zapret-discord-youtube) — Оптимизированные и настроенные готовые скрипты обхода под конкретные популярные сервисы.
* [ByeByeDPI](https://github.com/romanvht/ByeByeDPI) — Проверенное классическое решение для локального запуска на Windows.
* [tg-ws-proxy](https://github.com/Flowseal/tg-ws-proxy) — Инструмент для проксирования Telegram через WebSockets (гарантирует стабильную работу мессенджера в условиях суровых блокировок).

---

### 🌐 Обход «Белых списков» (TURN & WebRTC туннелирование)

*Решения для развертывания на VPS в условиях жестких сетевых ограничений и работы интернета строго по «белым спискам». Инструменты инкапсулируют и маскируют произвольный сетевой трафик (WireGuard, Hysteria, TCP) под легитимные медиапотоки видеозвонков и конференций (VK Звонки, Яндекс Телемост, WebRTC/SFU):*

* [csqtt](https://github.com/amurcanov/csqtt) — Высокопроизводительный сервер и Android-приложение для создания защищённого туннеля поверх TURN/RTP с маскировкой под медиатрафик голосовых/видеозвонков.
* [olcrtc](https://github.com/openlibrecommunity/olcrtc) — Зашифрованный TCP-over-WebRTC туннель, маскирующий подключение к VPS под стандартное участие в WebRTC/SFU видеоконференциях доверенных платформ.
* [vk-turn-proxy](https://github.com/cacggghp/vk-turn-proxy) — Good TURN: Серверный и клиентский проксификатор для проброса WireGuard / Hysteria через TURN-серверы VK Звонков и Яндекс Телемоста (DTLS 1.2 поверх STUN).
* [proxy-turn-vk-android](https://github.com/amurcanov/proxy-turn-vk-android) — Клиентское Android-приложение (WDTT) для туннелирования WireGuard на личный VPS через промежуточные медиарелеи VK TURN.
