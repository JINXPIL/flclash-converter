...
Чтобы не засорять страницу кодом, готовые пресеты настроек вынесены в отдельные файлы репозитория.

* 🌐 **Раздельное туннелирование для Happ** — Оптимизированный пресет правил (белые списки провайдеров Крыма, СевГУ, обход ограничений, AdBlock), собранный через конструктор `routing.happ.su`. Предназначен **строго** для утилиты Happ.
